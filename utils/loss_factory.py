...
class CELoss(torch.nn.Module):
    def __init__(self, ce_ls=0.):
        super(CELoss, self).__init__()
        self.ce_ls = ce_ls
        self.loss = torch.nn.CrossEntropyLoss(label_smoothing=ce_ls)
    def forward(self, y_pred, y_true):
...
def get_loss(loss1, loss2=None, alpha1=1., alpha2=0., ce_ls=0.):
    if loss1 == loss2 and alpha2 != 0.:
        warnings.warn('using same loss twice, you sure?')
    loss_dict = dict()
    loss_dict['bce'] = BCELoss(ce_ls)
    loss_dict['ce'] = CELoss(ce_ls)
